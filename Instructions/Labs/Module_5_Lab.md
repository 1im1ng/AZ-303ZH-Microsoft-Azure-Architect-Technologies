...
Adatum 公司在其本地存储中托管了大量非结构化和半结构化数据。它的维护变得越来越复杂和昂贵。为了满足数据保留要求，有些数据保存了很长时间。  Adatum Enterprise 体系结构团队正在寻找便宜的替代方案，以支持分层存储，同时允许安全访问，从而最大程度地减少数据泄露的可能性。尽管团队意识到 Azure 存储提供的容量几乎是无限的，但它仍担心帐户密钥的使用情况，该帐户密钥会授予对相应存储帐户所有内容的无限访问权限。尽管密钥可以有序地旋转，但执行这种操作需要有适当的计划。此外，访问密钥是一种专门授权机制，这限制了正确审核其使用情况的能力。

为了解决这些不足之处，体系结构团队决定探索共享访问签名的使用。共享访问签名 (SAS) 提供对存储帐户中资源的安全委派访问，同时将意外数据泄露的可能性降到最低。SAS 提供了对数据访问的精细控制，包括限制对单个存储对象（例如 blob）访问的能力，将此类访问限制到一个自定义时间窗口，以及筛选对指定 IP 地址范围的网络访问。此外，体系结构团队希望评估 Azure 存储与 Azure Active Directory 之间的集成级别，以解决其审核要求。体系结构团队还决定确定 Azure 文件存储作为一些本地文件共享的替代方案是否适合。

为了实现这些目标，Adatum 公司将测试 Azure 存储资源的一系列身份验证和授权机制，包括：
...
-  通过利用共享访问签名实现对 Azure 存储 Blob 的授权

-  通过利用 Azure Active Directory 实现对 Azure 存储 Blob 的授权

-  通过利用访问密钥实现对 Azure 存储文件共享的授权
...
1. 在 Cloud Shell 窗格中，通过运行以下命令创建资源组（用订阅中可用于部署 Azure VM 且最接近实验室计算机位置的 Azure 区域的名称替换 `<Azure region>` 占位符）：
   ...
   ```
      ...
      > **注**：若要识别可在其中配置 Azure VM 的 Azure 区域，请参阅 [**https://azure.microsoft.com/zh-cn/regions/offers/**](https://azure.microsoft.com/zh-cn/regions/offers/)
...
### 练习 1 ：通过使用共享访问签名配置 Azure 存储帐户授权。
...
1. 在与 **“az30302a-vm0”** 的远程桌面会话中，在浏览器窗口中的 **“共享访问签名”** 边栏选项卡上，指定以下设置（将其他设置保留为默认值）：
    ...
    | 允许的权限 | **读取**，**创建** |
    | Blob 版本控制权限 | 已禁用 |
    | 开始时间 | 在你所在时区当前时间之前 24 小时 | 
    | 结束时间 | 在你所在时区的当前时间之后 24 小时 |
    | 允许的协议 | **仅 HTTPS** |
    | 签名密钥 | **key1** |
...
#### 任务 6：使用 blob 级别共享访问签名来访问 blob

1. 在与 **az30302a-vm0** 的远程桌面会话中，在浏览器窗口中的 **“container1”** 边栏选项卡中选择 **“更改访问级别”**，验证是否将其设置为 **“私人（无匿名访问）”**，然后选择 **“取消”**。

    >**备注**：要允许匿名访问，可以将公共访问级别设置为 **“Blob（仅限 Blob 的匿名读取访问）”** 或 **“容器（容器和 Blob 的匿名读取访问）”**。
...
1. 在 Cloud Shell 窗格中运行以下命令，标识 Azure AD DNS 域名：
...
1. 在 Windows PowerShell 提示符后运行以下命令，使用在本练习的第一个任务中创建的 Azure AD 用户帐户对 AzCopy 进行身份验证。 
   ...
   ```
    ...
    >**备注**：不能将 Microsoft 帐户用于此目的，这是必须首先创建 Azure AD 用户帐户的原因。
...
1. 从 Windows 将驱动器映射到 Azure 存储文件共享

1. 删除实验室中部署的 Azure 资源


#### 任务 1：创建 Azure 存储和文件共享
...
#### 任务 2：从 Windows 将驱动器映射到 Azure 存储文件共享

1. 选择新创建的文件共享，然后选择 **“连接”**。

1. 在 **“连接”** 边栏选项卡上，请确保 **“Windows”** 选项卡处于选中状态，然后选择 **“复制到剪贴板”**。

    >**备注**：Azure 存储文件共享映射分别使用存储帐户名以及两个存储帐户密钥之一作为用户名和密码的等效项，以便获得对目标共享的访问权限。
